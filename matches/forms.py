...
class MatchUpdateForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Pre-fill selected_line_up and selected_substitutes if instance exists
        if self.instance and self.instance.pk:
            self.fields['selected_line_up'].initial = Player.objects.filter(name__in=self.instance.line_up)
            self.fields['selected_substitutes'].initial = Player.objects.filter(name__in=self.instance.substitutes)

    def save(self, commit=True):
        match = super().save(commit=False)
        
        # Convert QuerySet to a list of player IDs for JSONField storage
        match.line_up = list(self.cleaned_data['selected_line_up'].values_list('name', flat=True))
        match.substitutes = list(self.cleaned_data['selected_substitutes'].values_list('name', flat=True))
